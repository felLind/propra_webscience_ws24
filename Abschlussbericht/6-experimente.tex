...
\subsubsection{Klassische Ansätze}\label{subsubsec:klassische-ansaetze}
...
\subsubsection{Deep Learning Ansätze} \label{subsubsec:deep-learning-ansaetze}
...
\begin{quote}
    Tweet sentiment? Sentiment Topic: $Query-Term$\\
...
\begin{table}
    ...
    \caption{
        ...
        Die Modelle \textit{LR}, \textit{SVM} und \textit{NB} bezeichnen die Modelle logistische Regression, \textit{Support Vector Machine} und Naiver Bayes Klassifikator.
...
In Tabelle~\ref{tab:stats-per-model} sind die Statistiken der Testgenauigkeit für die Modelle \textit{LinearSVC}, \textit{LogisticRegression} und \textit{NaiveBayes} über alle Parameter-Kombinationen aufgeführt.
...
\begin{table}
    ...
    \begin{tabular}{lccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Statistiken der Testgenauigkeit über alle Parameter-Kombinationen für die Modelle \textit{SVM}, Logistische Regression und Naiver Bayes, die durch die Kürzel LR, SVM und NB dargestellt sind.}
...
\subsubsection{\textit{Deep Learning} Ansätze}
